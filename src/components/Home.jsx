...
class Home extends Component {
  ...
  countries = () => {
    return this.state.countries.length === 0 ? (
      ...
      this.state.countries.map((country) => (
        <Country key={country.capital} countryDetails={country} />
      ))
    );
  };
  changeMode = (mode) => {
  ...
  handleSearchCountryByRegion = (region) => {
    fetch(`https://restcountries.com/v2/region/${region.item.props.children}`)
      .then((response) => response.json())
      .then((countryDetails) => {
        this.setState({ countries: [] });
        this.setState({ countries: countryDetails });
    ...
    return (
      <ThemeContext.Provider value={this.state.theme}>
        ...
        >
          ...
          <div
            style={{
              background: this.state.theme.cardsBackground,
              padding: "1.7vw",
            }}
          >
            <Searchfilter
              handleSearchCounrty={this.handleSearchCounrty}
              handleSearchCountryByRegion={this.handleSearchCountryByRegion}
